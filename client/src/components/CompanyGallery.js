        ...
        {urls.slice(0, 6).map((u, i) => (
          ...
          >
            <img
              src={u}
              alt={`${companyName} ${i}`}
              className="w-full h-40 object-cover hover:scale-105 transition rounded-2xl"
            />
          </button>
        ))}

        {urls.length === 0 && (
          <div className="col-span-3 h-40 rounded-2xl bg-gray-200 flex items-center justify-center text-gray-500">
            Nema slika
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-6">
          <div className="relative w-full max-w-4xl">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 bg-gray-200 p-2 rounded-2xl flex justify-center items-center">
                <img
                  src={urls[idx]}
                  alt={`${companyName} big`}
            ...
            <div className="mt-3 flex gap-2 overflow-x-auto">
              {urls.map((u, i) => (
                <button
                  key={i}
                  onClick={() => setIdx(i)}
                  className={`w-20 h-16 overflow-hidden rounded-2xl ${i === idx ? "ring-2 ring-white" : ""}`}
                >
                  <img
                    src={u}
                    alt={`thumb ${i}`}
